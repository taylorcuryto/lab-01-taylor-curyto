...
There are 1846 rows and 3 variables: **datasat**, **x**, and **y**.
...
The correlation coeficient between x and y is -0.0645.
...
star_data %>%
  summarize(r = cor(x, y))
```
The correlation coefficients between x and y of the dino graph and star graph are very similar values. They are both negative values around -0.06. 
...
circle_data %>%
  summarize(r = cor(x, y))
```
The correlation coefficients between x and y of the dino graph and circle graph are very similar values. They are both negative values around -0.06. 
